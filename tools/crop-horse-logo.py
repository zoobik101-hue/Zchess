"""Trim Horse logo padding and export navbar shield-only asset."""
import struct
import zlib
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
SRC = ROOT / "assets" / "Image" / "Horse_in_a shield.png"
OUT_NAV = ROOT / "assets" / "Image" / "Horse_nav.png"
SHIELD_RATIO = 0.74
...
def trim_rows(rows, w, h, lum_min=18, pad_ratio_x=0.02, pad_ratio_y=0.04):
    top = h
    bottom = 0
    left = w
    right = 0
    for y, row in enumerate(rows):
        for x in range(0, w * 4, 4):
            ...
            if a > 8 and luminance(r, g, b) > lum_min:
                ...
                left = min(left, x // 4)
                right = max(right, x // 4)
    pad_x = max(4, int((right - left + 1) * pad_ratio_x))
    pad_y = max(4, int((bottom - top + 1) * pad_ratio_y))
    top = max(0, top - pad_y)
